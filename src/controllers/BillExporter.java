...
public class BillExporter {
    ...
    public static void export(int maDon) throws Exception {
        ...
        PdfWriter.getInstance(doc, new FileOutputStream(fileName)); // KHÔNG try-with-resources
        try {
            doc.open();
            ...
            try (Connection conn = DBConnection.getConnection()) {
                ...
                String sqlVe = """
                    SELECT p.ten_phim,
                           sc.bat_dau_luc,
                           v.gia_ban,
                           CONCAT(g.hang_ghe, g.so_ghe) AS ten_ghe
                    FROM don_ve dv
                ...
                try (PreparedStatement ps1 = conn.prepareStatement(sqlVe)) {
                    ps1.setInt(1, maDon);
                ...
                try (PreparedStatement ps2 = conn.prepareStatement(sqlSP)) {
                    ps2.setInt(1, maDon);
            ...
            if (doc.isOpen()) {
                doc.close(); // SẼ tự close PdfWriter + FileOutputStream
            }
        }
    }
